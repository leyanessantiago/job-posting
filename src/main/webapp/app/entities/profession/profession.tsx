...
export type IProfessionState = IPaginationBaseState;

export class Profession extends React.Component<IProfessionProps, IProfessionState> {
  state: IProfessionState = {
  ...
  render() {
    const { professionList, match, totalItems } = this.props;
    return (
      <div>
        ...
        <div className="table-responsive">
          {professionList && professionList.length > 0 ? (
            ...
            <div className="alert alert-warning">No Professions found</div>
          )}
        </div>
        <div className={professionList && professionList.length > 0 ? '' : 'd-none'}>
...
const mapStateToProps = ({ profession }: IRootState) => ({
  ...
  totalItems: profession.totalItems
});
